...
from vqdsr.utils import video_util
from vqdsr.utils.inference_base import get_base_argument_parser
from basicsr.data.transforms import mod_crop
from basicsr.utils.img_util import img2tensor, tensor2img
from basicsr.utils.logger import AvgTimer
from vqdsr.archs.vsr_wo_sr_arch import MSRSWVSR_WO_SR
def run_inference(input_video_path, output_dir, model_path, netscale=4, outscale=4, half=False, extract_frame_first=False, suffix=None, ema=True, num_process_per_gpu=1):
    # 创建一个类存储参数，替代命令行解析
    class Args:
        pass

    args = Args()
    args.input = input_video_path
    args.output = output_dir
    args.model_path = model_path
    args.netscale = netscale
    ...
    args.output = osp.join(args.output, 'inference_results', 'videos', args.video_name)
    ...
    # 返回生成的视频文件路径
    video_save_path = osp.join(args.output, f'{args.video_name}{args.suffix or ""}.mp4')
    return video_save_path

def get_inference_model(args, device):
    """return an on device model with eval mode"""
    # set up model
    model = MSRSWVSR_WO_SR(num_feat=64, num_block=[5, 3, 2], netscale=args.netscale)
...
def main():
    ...
    parser.add_argument(
        '--extract_frame_first',
        action='store_true',
        help='if input is a video, you can still extract the frames first, other wise VQD-SR will read from stream')
    parser.add_argument(
